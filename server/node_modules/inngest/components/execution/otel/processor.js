...
var InngestSpanProcessor = class InngestSpanProcessor {
	...
	*/
	declareStepExecution(rootSpanId, hashedStepId, attempt) {
		processorDebug("declareStepExecution: rootSpanId=%s hashedStepId=%s attempt=%d", rootSpanId, hashedStepId, attempt);
	...
	onEnd(span) {
		...
		try {
			if (this.#spansToExport.has(span)) {
				if (!this.#batcher) return debug("batcher not initialized, so failed exporting span", spanId);
