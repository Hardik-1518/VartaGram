...
var WebSocketWorkerConnection = class {
	...
	options;
	strategy;
	debugLog = Debug("inngest:connect");
	constructor(options) {
	...
	applyDefaults(opts) {
		...
		if (options.isolateExecution === void 0) {
			...
			if (envValue === "1" || envValue === "true") options.isolateExecution = true;
		}
		return options;
	}
	get state() {
	...
	*/
	async connect(attempt = 0) {
		...
		const hashedSigningKey = this.options.signingKey ? hashSigningKey(this.options.signingKey) : void 0;
		if (this.options.signingKey && this.options.signingKey.startsWith(InngestBranchEnvironmentSigningKeyPrefix) && !envName) throw new Error("Environment is required when using branch environment signing keys");
		const hashedFallbackKey = this.options.signingKeyFallback ? hashSigningKey(this.options.signingKeyFallback) : void 0;
		const capabilities = {
			trust_probe: "v1",
			connect: "v1"
		};
		...
		this.debugLog("Prepared sync data", { functionSlugs: Object.entries(functionConfigs).map(([appId, { functions }]) => {
